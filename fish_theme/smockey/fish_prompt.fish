# name: smockey
#
# smockey is a Powerline-style, Git-aware fish theme optimized for awesome.
#
...
function __smockey_in_git
  command git rev-parse --is-inside-work-tree >/dev/null 2>&1
end

function __smockey_git_branch
...
function __smockey_pretty_parent -d 'Print a parent directory, shortened to fit the prompt'
  echo -n (dirname $argv[1]) | sed -e 's|/private||' -e "s|^$HOME|~|" -e 's-/\(\.\{0,1\}[^/]\)\([^/]*\)-/\1-g' -e 's|/$||'
end

function __smockey_project_dir -d 'Print the current git project base directory'
  command git rev-parse --show-toplevel 2>/dev/null
end

function __smockey_project_pwd -d 'Print the working directory relative to project root'
  set -l base_dir (__smockey_project_dir)
...
function __smockey_start_segment -d 'Start a segment'
  set_color -b $argv[1]
  set_color $argv[2]
  if [ "$current_bg" = 'NONE' ]
...
function __smockey_path_segment -d 'Display a shortened form of a directory'
  if test -w "$argv[1]"
    __smockey_start_segment $dk_grey $med_grey
  else
    __smockey_start_segment $dk_red $lt_red
  end
  ...
  switch "$argv[1]"
    ...
    case "$HOME"
      set directory '~'
    case '*'
      set parent    (__smockey_pretty_parent "$argv[1]")
      set parent    "$parent/"
...
function __smockey_finish_segments -d 'Close open segments'
...
function __smockey_prompt_status -d 'the symbols for a non zero exit status, root and background jobs'
  set -l nonzero
  set -l superuser
  set -l bg_jobs
  ...
  if test "$nonzero" -o "$superuser" -o "$bg_jobs"
    __smockey_start_segment fff 000
    if [ "$nonzero" ]
...
function __smockey_prompt_user -d 'Display actual user if different from $default_user'
  if [ "$theme_display_user" = 'yes' ]
    if [ "$USER" != "$default_user" -o -n "$SSH_CLIENT" ]
      __smockey_start_segment $lt_grey $slate_blue
...
function __smockey_prompt_git -d 'Display the actual git state'
  ...
  __smockey_path_segment (__smockey_project_dir)

  __smockey_start_segment $flag_bg $flag_fg
  set_color $flag_fg --bold
  echo -n -s (__smockey_git_branch) $flags ' '
  set_color normal

  set -l project_pwd  (__smockey_project_pwd)
  if test "$project_pwd"
    if test -w "$PWD"
...
function __smockey_prompt_dir -d 'Display a shortened form of the current directory'
  __smockey_path_segment "$PWD"
end
...
function fish_prompt
  set -g RETVAL $status
  __smockey_prompt_status
  __smockey_prompt_user
  if __smockey_in_git
    __smockey_prompt_git
  else if __smockey_in_hg
    __smockey_prompt_hg
  else
    __smockey_prompt_dir
  end
  __smockey_finish_segments
end
