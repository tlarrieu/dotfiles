...
menu_binds = {
    -- Navigate items
    key({},          "t",       function (w) w.menu:move_down() end),
    key({},          "s",       function (w) w.menu:move_up()   end),
...
add_binds("all", {
    ...
    -- Tabs switching
    but({"Alt_L" },   4, function (w) w:prev_tab() end),
    but({"Alt_L" },   5, function (w) w:next_tab() end),
})
...
add_binds("normal", {
    ...
    key({},          ",",           function (w) w:set_mode("insert")  end),
    key({},          ".",           function (w) w:set_mode("command") end),
    key({},          "ê",           function (w) w:set_mode("search") end),

    --key({},          ":",           function (w) w:set_mode("command") end),

    -- Scrolling
    key({},          "t",           function (w) w:scroll{ y = more    } end),
    key({},          "s",           function (w) w:scroll{ y = less    } end),
    key({},          "c",           function (w) w:scroll{ x = less    } end),
    key({},          "r",           function (w) w:scroll{ x = more    } end),
    ...
    buf("^'$",                      function (w, c) w:enter_cmd(":tabopen ") end),
...
--    buf("^,h$",                     function (w, c) w:enter_cmd(":open duckduckgo !hackage ") end),
--    buf("^,n$",                     function (w, c) w:enter_cmd(":open duckduckgo !netflix ") end),
--    buf("^,w$",                     function (w, c) w:enter_cmd(":open wikipedia ") end),
    ...
    key({},          "B",           function (w, m) w:forward(m.count) end),
    ...
    key({         }, "S",     function (w)       w:prev_tab() end),
    key({         }, "T",   function (w)       w:next_tab() end),
    ...
    key({}         , "\"",          function (w) w.tabs:switch(1) end),
    key({}         , "«",           function (w) w.tabs:switch(2) end),
    key({}         , "»",           function (w) w.tabs:switch(3) end),
    key({}         , "(",           function (w) w.tabs:switch(4) end),
    key({}         , ")",           function (w) w.tabs:switch(5) end),
    key({}         , "@",           function (w) w.tabs:switch(6) end),

    -- Open tab from current tab history
    buf("^gy$",                     function (w) w:new_tab(w.view.history or "") end),

    key({},          "v",           function (w) w:reload() end),
...
for i=1,10 do
    table.insert(mod1binds,
        key({}, tostring(i % 10), function (w) w.tabs:switch(i) end))
end
